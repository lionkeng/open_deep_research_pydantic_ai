...
class EvaluationResult:
    ...
    output: ResearchResults | None = None
    error: str | None = None
    multi_judge_result: Any | None = None
...
class ResearchExecutorEvaluator:
    ...
    def load_dataset_from_yaml(self, categories: list[str] | None = None) -> list[TestCase]:
    ...
    def get_hardcoded_test_cases(self, categories: list[str] | None = None) -> list[TestCase]:
        ...
        edge_cases = [
            TestCase(
                name="minimal_query",
                category="edge_cases",
                input=ResearchExecutorInput(query="Python", complexity="simple"),
                expected=ResearchExecutorExpectedOutput(min_findings=2, max_findings=6),
                evaluators=["FindingsRelevanceEvaluator", "ComprehensiveEvaluator"],
            ...
            TestCase(
                ...
                expected=ResearchExecutorExpectedOutput(
                    min_findings=2, max_findings=5, expected_categories=["technical", "scientific"]
                ),
        ...
        performance_cases = [
            TestCase(
                ...
                input=ResearchExecutorInput(
                    query="What is the population of Tokyo in 2024?", complexity="simple"
                ),
                expected=ResearchExecutorExpectedOutput(
                    min_findings=1, max_findings=3, max_response_time=10.0
                ),
            ...
            async with httpx.AsyncClient() as http_client:
                ...
                for evaluator_name in test_case.evaluators:
                    if evaluator_name in self.evaluator_map:
                        ...
                        ctx = SimpleNamespace(
                            output=output, expected_output=test_case.expected, input=test_case.input
                        )
                ...
                if self.use_multi_judge:
                    ...
                    multi_judge_result = await self.multi_judge_evaluator.evaluate(
                        test_case.input.query, output, context
                    )
    ...
    async def run_evaluation(
        self, categories: list[str] | None = None, max_concurrency: int = 3
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"[green]Evaluating {len(test_cases)} cases...", total=len(test_cases)
            )
            ...
            results = await asyncio.gather(*[evaluate_with_semaphore(case) for case in test_cases])
    ...
    def calculate_summary(self, results: list[EvaluationResult]) -> dict[str, Any]:
        ...
        avg_evaluator_scores = {
            evaluator: sum(scores) / len(scores) for evaluator, scores in evaluator_scores.items()
        }
...
"""
        ...
        if summary["avg_evaluator_scores"]:
            ...
            for evaluator, score in sorted(
                summary["avg_evaluator_scores"].items(), key=lambda x: x[1], reverse=True
            ):
        ...
        if summary["category_results"]:
            ...
            for category, results in summary["category_results"].items():
                ...
                console.print(
                    f"  {category:20} {results['passed']}/{total} passed ({pass_rate:.1%})"
                )

    def save_results(
        self, results: list[EvaluationResult], summary: dict[str, Any], filename: str = None
    ):
...
async def main():
    ...
    parser.add_argument(
        "--multi-judge", action="store_true", help="Enable multi-judge consensus evaluation"
    )
    parser.add_argument(
        "--concurrency", type=int, default=3, help="Maximum concurrent evaluations (default: 3)"
    )
    parser.add_argument("--save", action="store_true", help="Save results to JSON file")
    parser.add_argument("--quick", action="store_true", help="Run quick test with only 3 cases")
    ...
    # Run evaluation
    console.print("[bold cyan]Starting Research Executor Agent Evaluation[/bold cyan]")
    if args.categories:
    ...
    results, summary = await evaluator.run_evaluation(
        categories=args.categories, max_concurrency=args.concurrency
    )
