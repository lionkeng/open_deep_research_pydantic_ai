...
async def test_single_case(case_name: str, category: str = None):
    ...
    print(f"   Expected: {'Clarification' if found_case['expected']['needs_clarification'] else 'No clarification'}")
    ...
    # Display results
    print(f"\n📊 Results:")
    print(f"   {'✅' if result['correct'] else '❌'} Prediction: {'Clarification' if result['predicted'] else 'No clarification'}")
    ...
    if result.get('question_analysis') and result['question_analysis']:
        ...
        print(f"   Required: {qa.get('required_count', 0)}, Optional: {qa.get('optional_count', 0)}")
