...
async def main():
    ...
    if clarification_yaml.exists():
        print(f"   ✓ Clarification dataset found")
    else:
        print(f"   ✗ Clarification dataset missing")

    if transformation_yaml.exists():
        print(f"   ✓ Query transformation dataset found")
    else:
        print(f"   ✗ Query transformation dataset missing")
    ...
    try:
        from tests.evals.clarification_evals import BinaryAccuracyEvaluator
        print("   ✓ Clarification evaluators loaded")
    except Exception as e:
        print(f"   ✗ Clarification evaluators error: {e}")

    try:
        from tests.evals.query_transformation_evals import SearchQueryRelevanceEvaluator
    ...
    if os.getenv("OPENAI_API_KEY"):
        ...
        try:
            from tests.evals.run_multi_judge_demo import evaluate_clarification_agent
            ...
            async with httpx.AsyncClient(timeout=10.0) as http_client:
                ...
                deps = ResearchDependencies(
                    ...
                    api_keys=APIKeys(
                        openai=SecretStr(os.getenv("OPENAI_API_KEY"))
                    ),
