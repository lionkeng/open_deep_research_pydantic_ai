"""Base Multi-Judge Evaluation Framework.

This module provides a generalized multi-judge consensus evaluation system
that can be used to evaluate any agent type with sophisticated voting mechanisms,
confidence weighting, and disagreement analysis.
"""
...
class AgentEvaluationAdapter(Protocol[TInput, TOutput]):
    ...
    def format_output_for_evaluation(self, output: TOutput) -> str:
        """Format agent output into a string for evaluation."""
        ...
    ...
    def is_output_valid(self, output: TOutput) -> bool:
        """Check if the output is valid for evaluation."""
        ...
...
class BaseMultiJudgeEvaluator(Generic[TInput, TOutput]):
    """Base class for multi-judge evaluation with consensus mechanisms."""
    ...
    def __init__(
        ...
        Args:
            adapter: Agent-specific evaluation adapter
        ...
        self.dimensions = adapter.get_evaluation_dimensions()
...
Be precise, objective, and consider the specific context when evaluating."""
    ...
    async def evaluate(
        ...
        if not self.adapter.is_output_valid(output):
        ...
        evaluation_prompt = self.adapter.create_evaluation_prompt(input, output, context)
            ...
            for judgment in valid_judgments:
                if dim_name in judgment.scores:
                    ...
                    if self.voting_method == VotingMethod.CONFIDENCE_WEIGHTED:
                        weight = judgment.confidence / 10.0
    ...
    def _analyze_agreement(
        ...
        for dimension in self.dimensions:
            ...
            if len(dim_scores) > 1:
                ...
                max_variance = (dimension.scale[1] - dimension.scale[0]) ** 2 / 4
    ...
    async def compare_outputs(
        ...
        formatted_a = self.adapter.format_output_for_evaluation(output_a)
        formatted_b = self.adapter.format_output_for_evaluation(output_b)
        ...
        Please compare these two outputs and determine which is better overall.
        Consider all evaluation dimensions and overall quality.
