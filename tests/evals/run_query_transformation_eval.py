...
class QueryTransformationEvaluator:
    ...
    def load_dataset_from_yaml(
        self, file_path: Path | None = None, categories: list[str] | None = None
    ) -> Dataset:
        """
        ...
        if file_path is None:
            file_path = (
                Path(__file__).parent / "evaluation_datasets" / "query_transformation_dataset.yaml"
            )
            ...
            if category_key in data:
                ...
                for case_data in category_cases:
                    try:
                        ...
                        expected_output = (
                            QueryTransformationExpectedOutput(**expected_data)
                            if expected_data
                            else None
                        )

                        # Create evaluators
                        evaluator_names = case_data.get(
    ...
    async def evaluate_query(
        self, query: str, complexity: str = "medium", domain: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        async with httpx.AsyncClient(timeout=30.0) as http_client:
            ...
            try:
                ...
                return {"query": query, "error": str(e), "response_time": None}

    async def run_evaluation_suite(self, categories: list[str] | None = None):
        """
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            result = await self.evaluate_query(
                test_case["query"], test_case["complexity"], test_case.get("domain")
            )
    ...
    def generate_report(self):
        ...
        if successful_results:
            ...
            print("  Search queries per query:")
            ...
            print(
                f"  Has methodology: {has_methodology}/{len(successful_results)} ({has_methodology / len(successful_results) * 100:.1f}%)"
            )
        ...
        with open(output_path, "w") as f:
            json.dump(
                {
                    "summary": {"total_cases": total, "successful": successful, "errors": errors},
                    "metrics": {
