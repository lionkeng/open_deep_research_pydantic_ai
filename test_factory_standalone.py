...
from src.open_deep_research_pydantic_ai.agents.factory import (
    AgentFactory,
    AgentPoolConfig,
    AgentType,
)
...
from src.open_deep_research_pydantic_ai.models.dependencies import ResearchDependencies
...
async def test_factory_standalone():
    ...
    pool_config = AgentPoolConfig(max_size=3, initial_size=1, enable_monitoring=True)
    ...
    # Test agent creation
    agent = await factory.create_agent(AgentType.RESEARCH_EXECUTOR, dependencies, use_pool=False)
    ...
    pooled_agent = await factory.create_agent(
        AgentType.RESEARCH_EXECUTOR, dependencies, use_pool=True
    )
